...
Recibe el DataFrame limpio generado por tablas.parsear_flujos() y devuelve
un DataFrame con la predicción, la confianza y el flag de baja confianza
para cada flujo, listo para ser enriquecido por siem.py.

Función principal:
    analizar_flujos(df_limpio, id_lote, ids_flujo) → DataFrame de predicciones
"""
...
_modelo, _nombres_caracteristicas, _umbrales_confianza = (
    cargar_artefactos.cargar_artefactos_ids()
)
...
def analizar_flujos(df_limpio, id_lote, ids_flujo):
    ...
    Parámetros:
        df_limpio  → DataFrame con las 24 columnas en el orden correcto,
                     ya normalizado por tablas.parsear_flujos()
    ...
    Proceso:
        1. Valida si el lote está vacío, que las columnas coincidan con las esperadas por el modelo, control de 
            dimensiones y reordena en el orden exacto que espera el modelo
        ...
        5. Compara la confianza con el umbral de la clase predicha
           para determinar el flag baja_confianza
    ...
    Devuelve:
        DataFrame con columnas:
            ...
            confianza      → probabilidad máxima asignada por el modelo (0-1)
    ...
    faltantes = [col for col in _nombres_caracteristicas if col not in df_limpio.columns]
    if faltantes:
    ...
    X = df_limpio[_nombres_caracteristicas].astype(float)

    predicciones = _modelo.predict(X)        
    ...
    predicciones = [
        _MAPEO_CLASES.get(p, p)  
        for p in predicciones
    ]

    probabilidades = _modelo.predict_proba(X)  

    confianzas = probabilidades.max(axis=1)    

    baja_confianza = []
    for pred, conf in zip(predicciones, confianzas):
        umbral = _umbrales_confianza.get(pred, 0.50) 
